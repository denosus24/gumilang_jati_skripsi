...
<aside
    ...
    {{
        $attributes->class([
            'fi-sidebar fixed inset-y-0 start-0 z-30 flex flex-col h-screen content-start bg-primary-900 transition-all dark:bg-gray-900 lg:z-0 lg:shadow-none lg:ring-0 lg:transition-none dark:lg:bg-transparent',
...
>
    <div class="overflow-x-clip">
        <header
            class="fi-sidebar-header flex h-16 items-center bg-primary-900 px-6 ring-1 ring-gray-950/5 dark:bg-gray-900 dark:ring-white/10 lg:shadow-sm"
        >
            <div
            ...
            >
                @if ($homeUrl = filament()->getHomeUrl())
                    @php $isDarkMode = true; @endphp
                    <a {{ \Filament\Support\generate_href_html($homeUrl) }}>
                        <x-filament-panels::selected-logo useTheme="dark" />
                    </a>
                @else
                    <x-filament-panels::selected-logo />
                @endif
            </div>
            ...
            @if (filament()->isSidebarCollapsibleOnDesktop() || filament()->isSidebarFullyCollapsibleOnDesktop())
                <x-filament::icon-button
                    ...
                    x-show="$store.sidebar.isOpen"
                    class="ms-auto hidden lg:flex text-white"
                />
            @endif
        </header>
    </div>
    ...
    <div x-persist="topbar.end" class="ms-5 py-5 flex items-center gap-x-4">
